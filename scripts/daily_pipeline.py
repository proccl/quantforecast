...
def is_hk_trading_day(date=None):
    """檢查指定日期是否是港股交易日"""
    if date is None:
        date = datetime.now()
...
def main():
    """主函數"""
    import pandas as pd  # 在這裡導入，避免 circular import
    
    today = datetime.now()
    
    print("\n" + "🔥" * 35)
    print(f"  QuantForecast 智能交易日回測")
    print(f"  檢查日期: {today.strftime('%Y-%m-%d %H:%M:%S')} ({['週一','週二','週三','週四','週五','週六','週日'][today.weekday()]})")
    print("🔥" * 35)
    
    # 檢查是否是交易日
    print("\n【步驟 0/3】檢查今天是否是港股交易日...")
    is_trading = is_hk_trading_day(today)
    
    if not is_trading:
        print(f"\n🌙 {today.strftime('%Y-%m-%d')} 不是港股交易日（週末或假期），跳過回測")
        return 0
    
    print(f"\n✓ {today.strftime('%Y-%m-%d')} 是港股交易日，繼續執行回測")
    ...
    duration = (end_time - today).total_seconds()
